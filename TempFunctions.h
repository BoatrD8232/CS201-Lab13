...
template <typename T> void mySwap(ostream& outFile, vector<T> values, int pos1, int pos2) {
...
template <typename T> T myMin(ostream& outFile, vector<T> values) {
	T min = values.at(0);
...
template <typename T> T myMax(ostream& outFile, vector<T> values) {
	T max = values.at(0);
	for (int i = 1; i < values.size(); ++i) {
		if (values.at(i) > max) {
			max = values.at(i);
...
template <typename T> int mySearch(ostream& outFile, vector<T> values, T target) {
	int pos = -1;
